...
Empfehlung (Stand 2026, bitte vor der Anmeldung die aktuellen Konditionen des Anbieters
prüfen, da sich Freemium-Angebote häufig ändern):

**Koyeb** (aktuell eine dauerhaft kostenlose, "always-on"-Instanz ohne Sleep):
1. Projekt auf GitHub hochladen (neues Repository erstellen, diese Dateien pushen).
2. Bei [koyeb.com](https://www.koyeb.com) registrieren, "Create Web Service" → GitHub-Repo auswählen.
3. Build/Run werden automatisch erkannt (Node.js, Startbefehl `node server.js` bzw. `npm start`).
4. Nach dem Deploy bekommst du eine feste Adresse wie `https://dein-app.koyeb.app` –
   diese Adresse funktioniert von überall, nicht nur im eigenen WLAN.

**Render** ist ebenfalls einfach einzurichten, hat auf dem kostenlosen Tier aber zwei
bekannte Einschränkungen, die bei einer Party stören können: Der Dienst legt sich nach
15 Minuten Inaktivität schlafen (nächster Aufruf dauert dann ~30–60 Sekunden zum
Aufwachen), und WebSocket-Verbindungen wurden auf dem Gratis-Tier in der Vergangenheit
nach kurzer Zeit getrennt. Für eine Party, bei der alle durchgehend verbunden bleiben
sollen, ist ein Anbieter mit echtem "Always-on" (wie Koyeb, oder ein bezahlter
Render-Plan) die zuverlässigere Wahl.
